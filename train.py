...
def gain_root(df_left, df_right, disc_index, disc_data, disc_coef, acc_coef):
    # print(type(root_data))
    pred_l = 0
    pred_r = 0
    delta_d = -float('inf')
    disc_data_best = disc_data

    disc_data_cpy = np.copy(disc_data)

    pred_left = round(np.mean(df_left[:,-2]))
    pred_right = round(np.mean(df_right[:,-2]))
    disc_old = disc_func.disc_fast(disc_data)
    ...
    acc_old = (np.sum(disc_data[:,0,0])+np.sum(disc_data[:,1,1]))/np.sum(disc_data[:,:,:])
    for pred_left in [0,1]:
        for pred_right in [0,1]:
            disc_data = np.copy(disc_data_cpy)
            for row in df_left:
            ...
            disc_new = disc_func.disc_fast(disc_data)
            if disc_data.size == 0 or np.sum(disc_data[0,:,:]) == 0 or np.sum(disc_data[1,:,:])==0 or np.sum(disc_data[1,1,:]) == 0 or np.sum(disc_data[0,1,:]) == 0:
                return -100, disc_data
            acc_new = (np.sum(disc_data[:,0,0])+np.sum(disc_data[:,1,1]))/np.sum(disc_data[:,:,:])
            disc_delta = disc_new-disc_old
            acc_delta = acc_new-acc_old
            # score = [1,-1][int(disc_delta)<0]*2*(disc_delta)**2-[1,-1][int(acc_delta)<0]*acc_delta**2
            score = disc_coef*disc_delta+acc_coef*acc_delta
            if score > delta_d:
                # print(score, acc_delta, acc_new, acc_old)
                delta_d = score
                pred_l = pred_left
                pred_r = pred_right
                disc_data_best = np.copy(disc_data)
    ...
    # if delta_d > 0:
    # print(delta_d)
    return delta_d, disc_data_best, pred_l, pred_r
...
class DecisionTreeFair:
    ...
    '''
    def __init__(self, disc_data, min_samples_split=2, max_depth=5, train_method=None, disc_index=None, acc_coef=1, disc_coef=2):
        ...
        for f_idx in range(n_cols-2):
            ...
            for threshold in np.unique(unique):
                ...
                if len(df_left) > 0 and len(df_right) > 0:
                    gain, disc_data, pred_l, pred_r = gain_root(df_left, df_right, self.disc_index, np.copy(self.disc_data), self.disc_coef, self.acc_coef)
                    if gain > best_gain:
